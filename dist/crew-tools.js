...
export function registerCrewTools(ctx, crews) {
    return [
        ctx.tools.register({
            name: 'crew_handoff',
            description: 'Hand one crew member\'s completed work to the next role as its next turn. Each role runs on its own preset and completes its task turn before waiting for the next handoff. The target role receives the task as its next inbox turn. In pipeline mode the to_role must be the deterministic successor (verifier may loop to predecessor on failure).',
            parameters: {
                type: 'object',
                properties: {
            ...
            async execute(args, exec) {
                ...
                const handoff = await crews.handoff(a.crew, a.from_role, a.to_role, [{ type: 'text', text: a.task }], exec.agent, exec.signal);
                return { ok: true, to_role: handoff.toRole };
            },
        }),
        ctx.tools.register({
            name: 'crew_materialize',
            description: 'Materialize every role of a named crew as continuously-resident worker agents (one per role preset). Call once before handing off work. Idempotent: already-live members are reused.',
            parameters: {
                type: 'object',
        ...
        ctx.tools.register({
            name: 'crew_status',
            description: 'List the named crews, their roles, orchestrator, mode, the effective per-role model routes (request-level agentOptions over the plugin settings), live members, pipeline order and verify-gate.',
            parameters: {
                type: 'object',
                properties: {},
                additionalProperties: false,
            ...
            execute() {
                return Promise.resolve({
                    crews: crews.listCrews().map((name) => {
                        ...
                        const roleRoutes = {};
                        for (const role of crews.roles(name))
                            roleRoutes[role] = crews.roleRoute(name, role);
                        return {
                            name,
        ...
        ctx.tools.register({
            ...
            async execute(args, exec) {
                const a = args;
                if (exec.agent === undefined)
                    throw new Error('crew_pipeline_advance requires a calling agent');
                const h = await crews.pipelineAdvance(a.crew, a.from_role, [{ type: 'text', text: a.task }], exec.agent, exec.signal, { taskId: a.task_id, verifierVerdict: a.verifier_verdict, evidence: a.evidence });
                return { ok: true, to_role: h.toRole, gate: h.gate };
            },
        }),
        ctx.tools.register({
            name: 'crew_pipeline_status',
            description: 'Read the pipeline cursor, verify-gate state and per-role structured tasks for one crew. Use to decide the next handoff or to render progress.',
            parameters: {
                type: 'object',
        ...
        ctx.tools.register({
            ...
            description: 'Update one structured task on a crew role (status, title, description, acceptanceCriteria). Tasks carry id/status/acceptance so the verifier gate can check them.',
            parameters: {
                type: 'object',
                properties: {
                    crew: { type: 'string', description: 'The crew name.' },
        ...
        ctx.tools.register({
            name: 'crew_verify',
            description: 'Record a verifier\'s structured pass/fail for a task. Updates the task to done/failed and, in pipeline mode, advances or loops the pipeline (respecting maxRetries). Use when the verifier has produced a verdict but hasn\'t handed off yet.',
            parameters: {
                type: 'object',
                properties: {
                    crew: { type: 'string', description: 'The crew name.' },
        ...
        ctx.tools.register({
            ...
            async execute(args, exec) {
                ...
                const settled = await Promise.all(members.map(({ role, member }) => waitForSettlement(ctx, String(member.childId), exec.signal).then((result) => ({ role, ...result }))));
                return { settled };
            },
        }),
        ctx.tools.register({
            name: 'subagent_wait',
            description: 'Block the current turn until a background continuable subagent (by subagent id) settles, then return its stop reason and closing output. Use this instead of sleeping or polling: it wakes the moment the subagent reports done, however long that takes.',
            parameters: {
                type: 'object',
