...
/** A partial model route: the three fields a delegation may pin. */
...
export function resolveRoute(
  request: RouteOverrides | undefined,
  config: RouteOverrides,
): RouteOverrides {
  const effective: { provider?: string; model?: string; maxTokens?: number } = {}
  const provider = request?.provider ?? config.provider
  const model = request?.model ?? config.model
  const maxTokens = request?.maxTokens ?? config.maxTokens
  if (provider !== undefined) effective.provider = provider
  if (model !== undefined) effective.model = model
  if (maxTokens !== undefined) effective.maxTokens = maxTokens
  return effective
}
...
export function roleRouteOverrides(role: {
  readonly agentOptions?: RouteOverrides | undefined
...
export function effectiveMaxDepth(
  ...
  configMax: number | 'provider-managed' | undefined,
): number | undefined {
  const configured = typeof configMax === 'number' ? configMax : undefined
  if (requestMax === undefined) return configured
  if (configured === undefined) return requestMax
  return Math.min(requestMax, configured)
}

/** Drop undefined-valued keys so a spread cannot shadow an inherited field. */
function omitUndefined<T extends Record<string, unknown>>(input: T): { [K in keyof T]?: Exclude<T[K], undefined> } {
