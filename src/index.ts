...
/** The one method this plugin needs from whichever settings generation is live. */
...
export function installSettings<T>(
  ...
  ctx.inject(['settings'], (settingsCtx) => {
    const settings = (settingsCtx as Context).get('settings') as unknown as SettingsWithSection | undefined
    ...
    void import('@deepseek-ai/dsh-settings').then((mod) => {
      const legacy = (mod as { installSettingsSection?: (...args: unknown[]) => void }).installSettingsSection
      if (typeof legacy === 'function') {
        legacy(ctx, ns, schema, entry, hooks)
        return
      }
      ctx.logger?.warn?.(
        `${name}: this harness exposes neither settings.installSection() nor installSettingsSection();`
