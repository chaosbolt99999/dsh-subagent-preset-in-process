 ...
 */
export function childMeta(parent: Agent, childDepth: number, isSeeded: boolean): Record<string, unknown> {
  const produced = childSessionMeta(parent, childDepth, (isSeeded ? 1 : 0) as never) as unknown as Record<string, unknown>
  ...
  start(request: ResolvedSubagentStartRequest): Promise<SubagentRun> {
    ...
    return parent.ctx.agents
      .create({
        ...
        setup: async (childCtx: Context, createdAgent?: Agent): Promise<void> => {
          ...
          const child = createdAgent ?? (childCtx as unknown as { agent?: Agent }).agent
          if (child === undefined) {
