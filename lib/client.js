...
function apply(ctx) {
  ...
  const remote = ctx.remote?.session === void 0 ? void 0 : ctx.remote;
  ctx.slots.inject(
    "settings.plugin.item",
    () => ctx.slots.register(
      { name: "settings.plugin.item", key: SETTINGS_NS, inject: () => ({ scope, remote }) },
      (props) => React.createElement(Card, { scope: props.scope, remote: props.remote })
    )
  );
}
